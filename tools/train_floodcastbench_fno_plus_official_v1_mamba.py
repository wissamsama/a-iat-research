...
def run_suffix(config: dict) -> str:
    return safe_name(config.get("experiment", {}).get("name", "fcb_fno_plus_official_v1_mamba_latent_highfid_60m"))


def build_model(config: dict) -> FNOPlusOfficial3dMamba:
...
def dry_run(config: dict) -> None:
    ...
    print("=== RESOLVED OFFICIAL FNO+ V1 NORMALIZED MAMBA CONFIG ===")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Train official FNO+ v1 normalized latent-Mamba reproduction experiment.")
