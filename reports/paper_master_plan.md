...
| 1.2 | WP12 Phase 2 (dose-réponse {Δt × cible}, 8 runs one-step courts) — décide si le pilier B est "mécanisme montré" ou "hypothèse honnête" | ~1-2 j GPU | **PAS "prêt à lancer" comme noté** — vérifié 2026-07-19 : aucun knob Δt/frame-stride n'existe dans le dataset (V1, frozen, slicing contigu `frames[start:start+window_length]`, aucun `frame_stride`). Il faut l'ajouter dans la sous-classe V2 (ne pas toucher V1, gelé par convention) avant de pouvoir lancer les 8 runs. Décision : **pas implémenté cette nuit** — ce code alimente directement la revendication mécanistique la plus fragile du papier (marquée CRITIQUE), une erreur d'indexation y serait très coûteuse à découvrir tard. À faire en journée, testé soigneusement (vérifier timestamps/Δt effectif sur un cas jouet) avant de lancer les 8 runs réels |
| 1.3 | Re-run protocole COMPLET (13/13) de la table Δ-Diff-vs-FNO+ dense | ~3-5h GPU | **FAIT 2026-07-19** — V2 dense 13/13, 3 seeds : relRMSE 0.001558 ± 0.000836 (mean sample std), NSE 0.999996 ± 0.000004, Pearson r 0.999999 ± 0.000001, CSI@0.001 0.985861 ± 0.003884, CSI@0.01 0.999063 ± 0.000416 — bat FNO+ publié sur les 5 métriques, quasi identique à la lecture rapide 4/13 (0.001576 ± 0.000818), même conclusion. Table centrale (dense row, ×3.7 vs jumeau) et table vs-FNO+ mises à jour dans le papier, mention "4-window screening" retirée, papier recompilé sans erreur |
...
| Transfert zero-shot Pakistan→Mozambique 480m (Table 5) | **EN PAUSE 2026-07-19 13:35** — configs créés, delta-stats Pakistan calculées, smoke test 1 epoch propre (loss finie et décroissante), entraînement complet lancé (seed42) et arrêté proprement (SIGTERM) à epoch ~24/300 pour libérer la machine P7 (réclamée par un autre étudiant). `rollout_val_rmse` décroissait sainement (0.00263→0.00207→0.00176→0.00144 aux epochs 5/10/15/20), aucun signe de plateau. `checkpoint_last.pth`/`checkpoint_best.pth` conservés sur disque mais **le trainer n'a pas de reprise automatique** (même limite que documentée pour FNO+/WPB1) — une relance repart d'epoch 0, pas d'epoch 24. À relancer dès que le GPU P7 est de nouveau disponible. | Nécessite un entraînement in-domain sur Pakistan d'abord (contrairement à UK, aucun checkpoint Pakistan n'existait). **Coût révisé à la hausse après mesure directe** : ~1565s/epoch (~26 min) — grille Pakistan 810×441, plus grande qu'Australie (536×536) et bien plus grande qu'UK (85×137). À budget epochs=300/patience=60 identique aux autres jumeaux, ça peut prendre plusieurs jours si l'early-stop ne se déclenche pas tôt. Protocole gardé identique (pas de raccourci d'epochs) pour rester comparable aux autres trainings jumeau — ancienne estimation "~1-2j" dans l'action 4 ci-dessous était fausse, corrigée ici après mesure réelle.
