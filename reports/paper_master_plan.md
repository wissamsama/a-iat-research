...
| Transfert zero-shot Pakistan→Mozambique 480m (Table 5) | **En cours 2026-07-19** — configs créés, delta-stats Pakistan calculées, smoke test 1 epoch propre (loss finie et décroissante), entraînement complet lancé (seed42, 1 seed d'abord) avec enchaînement automatique vers l'éval in-domain Pakistan puis le zero-shot Mozambique | Nécessite un entraînement in-domain sur Pakistan d'abord (contrairement à UK, aucun checkpoint Pakistan n'existait). **Coût révisé à la hausse après mesure directe** : ~1565s/epoch (~26 min) — grille Pakistan 810×441, plus grande qu'Australie (536×536) et bien plus grande qu'UK (85×137). À budget epochs=300/patience=60 identique aux autres jumeaux, ça peut prendre plusieurs jours si l'early-stop ne se déclenche pas tôt. Protocole gardé identique (pas de raccourci d'epochs) pour rester comparable aux autres trainings jumeau — ancienne estimation "~1-2j" dans l'action 4 ci-dessous était fausse, corrigée ici après mesure réelle. Tourne en arrière-plan, ne bloque rien d'autre.
