...
class FloodCastBenchFNOPlusOfficialDataset(Dataset):
    ...
    Returned tensors (context_length=0, the default / paper-matching case):
        x: [6, H, W, 20]
           channels are X, Y, T, initial water depth, DEM, rainfall.
        target: [1, H, W, 19]
           water depth for t=2..20.

    With context_length=K > 0 (see reports/fno_plus_beat_paper_plan.md WPB0),
    the window grows to K+20 frames: the first K+1 frames are REAL observed
    depths (K history frames plus the "current" frame, at their true time
    positions -- not broadcast), and the depth channel over the remaining 19
    target positions continues to broadcast the last known ("current") frame
    forward, exactly matching the K=0 convention for those positions. This
    keeps K=0 bit-for-bit identical to the original single-frame-context
    behavior (regression-tested) while giving K>0 the model real history
    instead of a repeated constant.
    """
    ...
    def __init__(
        ...
        self.window_length = self.context_length + self.sample_length
        ...
        self.time = torch.linspace(0.0, 1.0, self.window_length).view(1, 1, self.window_length)
    ...
    def __getitem__(self, index: int):
        ...
        # Real observed depths at the K+1 known positions (K history frames +
        # the "current" frame); the last known value is then broadcast across
        # the 19 target positions (never using target/future values as input).
        known_count = self.context_length + 1
        future_len = self.sample_length - 1
        history_and_current = np.stack(water[:known_count], axis=-1)
        future_broadcast = np.repeat(water[self.context_length][..., None], future_len, axis=-1)
        depth = torch.from_numpy(np.concatenate([history_and_current, future_broadcast], axis=-1)).float()
        ...
        target = torch.from_numpy(np.stack(water[known_count:], axis=-1)).float().unsqueeze(0)

        x_coord = self.xy[0].unsqueeze(-1).expand(self.height, self.width, self.window_length)
        y_coord = self.xy[1].unsqueeze(-1).expand(self.height, self.width, self.window_length)
        t_coord = self.time.expand(self.height, self.width, self.window_length)
        dem = dem.unsqueeze(-1).expand(self.height, self.width, self.window_length)
        x = torch.stack([x_coord, y_coord, t_coord, depth, dem, rainfall], dim=0).float()
        ...
        meta = {
            ...
            "input_timestamp": int(frames[self.context_length].timestamp),
            "history_timestamps": [int(frame.timestamp) for frame in frames[:self.context_length]],
            "target_timestamps": [int(frame.timestamp) for frame in frames[known_count:]],
    ...
    def _split_sample_starts(self, split_counts: dict[str, int] | None) -> list[int]:
        all_starts = [
            ...
            if start + self.window_length <= len(self.frames)
        ]
