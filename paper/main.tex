...
on two independent flood events), so any sampler whose residual noise
scales with its generative target is floored above the true signal. A
delta-space, regime-aware reparametrization removes this floor, and the
resulting model outperforms the strongest published neural-operator
baseline on all five reported metrics under full observation. Against its
...
\subsection{Why the absolute field cannot be diffused at fine cadence}
\label{sec:mechanism}
...
all). We trace this to a scale mismatch, not an implementation defect.
The true physical signal between two consecutive frames is tiny relative
to the field's own amplitude: writing $\sigma_\Delta$ for the standard
deviation of per-step changes and $\sigma_{\text{field}}$ for that of the
wet-pixel depths, we measure, directly from the raw data of two
independent events,
\begin{equation}
\sigma_{\text{field}} / \sigma_\Delta \;\approx\; 488 \text{ (Australia)}, \qquad 425 \text{ (UK)}
\end{equation}
(Figure~\ref{fig:mechanism}). A diffusion sampler's residual error scales
with the amplitude of \emph{its generative target}. Diffusing the absolute
field makes that target the full field amplitude, so merely matching
persistence requires per-step sampling error below $\sim$0.2\% of the
field's own scale --- beyond what a practical sampler achieves here.
\ours{} instead diffuses the \emph{per-step change} (delta), for which the
equivalent condition is trivially satisfied. This mechanism predicts that
the failure is a property of the parametrization, not of diffusion itself
--- exactly what the corrected model's results confirm
(\S\ref{sec:paramresults}).

\begin{figure}[t]
  \centering
  \includegraphics[width=0.92\linewidth]{figures/f2_mechanism.pdf}
  \caption{The mechanism behind absolute-field failure, measured from raw
  training data of two independent flood events. (a) The distribution of
  per-step changes $|\Delta|$ sits two to three orders of magnitude below
  the wet-pixel depth distribution. (b) The resulting scale gap
  ($\sim$488$\times$ Australia, $\sim$425$\times$ UK) is a lower bound on
  the relative sampling accuracy an absolute-field diffusion model must
  reach just to match persistence.}
  \label{fig:mechanism}
\end{figure}
...
it --- exactly as the mechanism of \S\ref{sec:mechanism} predicts. Second,
the short-context ablation confirms the advantage is not driven by input
history (\S\ref{sec:vsfnoplus} showed the same for the baseline).
...
Figure~\ref{fig:calibcomp} makes the comparison at \mfifty{} on identical
metrics and finite-ensemble corrections. The result is symmetric and
sobering: the twin ensemble ($M{=}3$) covers 46--47\% of its corrected
nominal intervals, statistically comparable to \ours{} ($M{=}8$, 43--48\%
under the closest available mask condition) --- \textbf{neither family
provides reliable uncertainty out of the box}, and the rank histograms
show the diffusion ensembles are, if anything, more overconfident in shape
than the twin ensemble. On this benchmark, calibration as measured so far
does not supply the generative model's missing justification.
...
\begin{figure}[t]
  ...
  ensemble of deterministic twins. (a) Observed over nominal coverage
  (finite-ensemble corrected; 1.0 = perfectly calibrated) --- every
  condition is severely under-covered, twin ensemble and \ours{} alike.
  (b) Rank histograms over active pixels: both are U-shaped
  (overconfident), \ours{} more strongly so. Caveats: the twin ensemble
  uses random masks at \mfifty{} while the nearest available \ours{}
  evaluations use structured masks (the gauge mask is behaviorally closest
  to random, \S\ref{sec:masks}); member counts differ ($M{=}3$ vs
  $M{=}8$); single-seed \ours{} evaluations. A fully matched comparison
  (random masks, all sparsities, corrected checkpoints) is pending.}
  \label{fig:calibcomp}
\end{figure}

\blocked{Remaining for this subsection: the \mninetyfive{} twin-ensemble
evaluation (waiting on the corrected \mninetyfive{} checkpoints), the
matched random-mask \ours{} evaluation at \mfifty{}, and a sharpness-aware
score (CRPS) to complement pure coverage.}
