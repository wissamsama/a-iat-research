...
% ---- arrows: all orthogonal (right angles only), inputs routed through a
% single shared vertical bus so wires from boxes of different widths still
% bend at the same x (a fixed relative jog would misalign, since xin/min/
% dem/rain all have different .east x-coordinates) ----
\coordinate (busin) at ($(encx)+(-3mm,0)$);
\draw[arr] (xin.east) -- (busin -| xin.east) -- (busin -| tenc.west) -- (tenc.west);
\draw[arr] (min.east) -- (busin -| min.east) -- (busin -| tenc.west) -- (tenc.west);
\draw[arr] (min.east) -- (busin -| min.east) -- (busin -| senc.west) -- (senc.west);
\draw[arr] (dem.east) -- (busin -| dem.east) -- (busin -| senc.west) -- (senc.west);
\draw[arr] (rain.east) -- (busin -| rain.east) -- (busin -| senc.west) -- (senc.west);
