...
\begin{enumerate}
  ...
  \item \textbf{A quantified failure pattern} for absolute-field
    diffusion at fine cadence: a 450--490$\times$ target-scale mismatch
    measured on two independent events, with an 8-point dose--response
    test confirming the predicted monotone decay (8/8) and a matching
    power-law exponent --- pending one control (\S\ref{sec:mechanism})
    to determine whether this is diffusion-specific or a more general
    parametrization effect.
...
\blocked{The cross-region transfer section (\S\ref{sec:zeroshot}) now
has two independent transfer pairs, evaluated on each target event's full
frame sequence (Australia$\to$UK, 3 seeds, $\sim$8.6$\times$;
Pakistan$\to$Mozambique, 1 seed, $\sim$21$\times$), both favoring the
twin by a wide margin over the published FNO+ numbers. What remains
before freeze is the 3-seed replication of the Pakistan$\to$Mozambique
pair, to bring it to the same statistical footing as the UK one rather
than changing this introduction's claims.}
...
trivially met. Two events with a high ratio \emph{and} a failure is
consistent with this hypothesis but does not demonstrate it --- a
dose--response test, reported next, does.
...
\paragraph{Dose--response result.} One fixed skeleton (\ours{}'s), a
single crossed factor \{$\Delta t \in$ 300, 600, 900, 1200, 1800, 2400,
4800, 7200\,s\} $\times$ \{target: absolute, delta\} --- 16 short
single-step training runs in the dense regime (no rollout confounds),
seed 42, screening-labeled --- give the absolute-arm-to-delta-arm error
ratio at eight $\Delta t$ values spanning a factor 24$\times$
(Table~\ref{tab:doseresponse}).

\begin{table}[H]
\centering
\caption{Dose--response: relative error of the absolute- and delta-target
arms (dense, one-step, seed 42, screening) at each tested $\Delta t$, and
their ratio. \textbf{Monotone decrease at every step, 8/8}, the
pre-registered criterion.}
\label{tab:doseresponse}
\begin{tabular}{rrrr}
\toprule
$\Delta t$ (s) & \absdiff{} RMSE (m) & \ours{} RMSE (m) & Ratio (\absdiff{}/\ours{}) \\
\midrule
...
The ratio decreases monotonically at every single step across the full
range --- the pre-registered criterion, met without exception. Beyond
monotonicity, the ratio follows the predicted power-law form closely: a
log--log regression of ratio against $\Delta t$ gives slope $-1.06$
($R^2=0.98$), matching the $-1$ exponent the sampling-noise-floor model
predicts (since $\sigma_\Delta$ grows near-linearly in $\Delta t$,
Figure~\ref{fig:doseresponse}).

\blocked{An important caveat surfaced after this fit was obtained, before
upgrading $\S$\ref{sec:mechanism}'s claim to ``demonstrated mechanism'':
the $-1$ slope may be a mechanical consequence of two facts unrelated to
diffusion sampling specifically, rather than evidence for the
diffusion-specific sampling-noise-floor model as stated. (i) \absdiff{}'s
RMSE is nearly \emph{constant} across the entire $\Delta t$ range
(0.368--0.397\,m, $\sim$8\% variation) --- it does not visibly respond to
$\Delta t$ at all. (ii) \ours{}'s RMSE divided by the independently
measured $\sigma_\Delta(\Delta t)$ (Figure~\ref{fig:doseresponse}) is
itself nearly constant ($\approx$0.22--0.25 across 4 of 5 directly
comparable points) --- i.e., the delta-arm's error simply tracks the
target's own scale, a generic regression property with nothing specific
to diffusion. If \absdiff{}'s error is $\Delta t$-invariant and
\ours{}'s error is proportional to $\sigma_\Delta(\Delta t)$, the ratio
$\propto 1/\sigma_\Delta(\Delta t)$ falls out mechanically from the
already-measured $\sigma_\Delta(\Delta t)\propto\Delta t$ scaling, without
requiring anything diffusion-specific. \textbf{Decisive control (running
at the time of writing):} the identical 8-point $\{\Delta t \times
\text{target}\}$ design on \twin{} (no diffusion sampling at all). If
\twin{} reproduces the same power law, the mechanism is a
\emph{parametrization} effect --- consistent with, and supporting, this
paper's central attribution thesis, but requiring $\S$\ref{sec:mechanism}
and this section to be reframed accordingly rather than left as a
diffusion-specific claim.} A secondary, unplanned finding regardless of
this caveat's resolution: \ours{} itself (delta target) stops beating
persistence beyond $\Delta t\approx 65$\,min (interpolated crossing
$\approx$3910\,s) --- at that horizon genuine physical change accumulates
enough that ``predict no change'' is no longer a weak baseline,
independent of target encoding. We also flag that this fit comes from a
single-seed screening protocol whose training-set size shrinks sharply at
the largest $\Delta t$ values (1579 training deltas at 300\,s vs.\ 65 at
7200\,s), so the extreme point anchoring one end of the fit is also the
least-supported by training data.
...
\begin{figure}[H]
  ...
  478$\to$21 (Australia) and 412$\to$17 (UK). The eight marked $\Delta t$
  values are the training points used in Table~\ref{tab:doseresponse},
  which confirms the predicted monotone closing (8/8) and a matching
  power-law exponent; the parity region (ratio $\approx$1) remains
  unreachable by subsampling alone, which bounds what this data-only
  panel can show on its own.}
...
\paragraph{A small-sample pitfall we caught before trusting the first
number.} Our initial transfer evaluations reused this paper's usual
20\%-held-out test split of the target event: 3 windows for UK, 7 for
Mozambique. Because a zero-shot checkpoint was never trained on \emph{any}
frame of the target event, restricting evaluation to that held-out
fraction is unnecessary and, on these two small target events, misleading
--- both slices happened to land on unrepresentatively calm portions of
the record. Re-evaluating on the target event's \emph{entire} frame
sequence (42 windows for UK, 85 for Mozambique) revealed the initial
ratios were substantially inflated: $\sim$37$\times$ dropped to
$\sim$9$\times$ for UK, $\sim$210$\times$ dropped to $\sim$21$\times$ for
Mozambique. We report only the full-event numbers below and flag the
small-sample pitfall explicitly, since a reader re-deriving our result on
the committed test-split configs would reproduce the inflated numbers, not
these.

\begin{table}[H]
\centering
\caption{Zero-shot cross-regional transfer, relative RMSE against the
benchmark's published FNO+ transfer result under the same frozen-weights
protocol, evaluated on each target event's \emph{full} frame sequence
(not the usual held-out test split --- see text). \twin{} is single-pass
deterministic output, matching FNO+'s output type.}
\label{tab:zeroshot}
\begin{tabular}{llrr}
\toprule
Transfer pair & Model & relRMSE $\downarrow$ & vs.\ FNO+ published \\
\midrule
Australia$\to$UK (42 windows) & FNO+ \citep{floodcastbench2025} & 0.024771 & --- \\
 & \twin{} (3 seeds) & $\mathbf{0.002874 \pm 0.000177}$ & $\sim$\textbf{8.6$\times$ better} \\
\midrule
Pakistan$\to$Mozambique (85 windows) & FNO+ \citep{floodcastbench2025} & 0.078633 & --- \\
 & \twin{} (1 seed) & $\mathbf{0.003748}$ & $\sim$\textbf{21.0$\times$ better} \\
\bottomrule
\end{tabular}
\end{table}

Both published transfer pairs point the same direction, with a much
larger and more representative evaluation sample than our first pass:
the twin transfers dramatically better than the published FNO+ baseline
on both independent regions. Two qualifications remain. First, the
Pakistan$\to$Mozambique row is still a \textbf{single seed}
(Australia$\to$UK is 3, with low seed variance: std $\approx$6\% of the
mean); the Pakistan in-domain training this depends on has not yet been
replicated at 3 seeds, in progress at the time of writing. Second, we
have verified our transfer protocol matches the published description
(frozen weights, adjusted target domain), but the publication does not
specify whether its normalization statistics were also frozen from the
source domain as ours are; if that choice differed, the comparison is
between closely related but not identical protocols. \blocked{\ours{}'s
own full-event zero-shot numbers (both pairs) and the Pakistan seed7/123
replication are running at the time of writing.}
...
beats FNO+ in-domain ($\sim$9.4$\times$) and in zero-shot transfer on
both published transfer pairs ($\sim$8.6$\times$ and $\sim$21$\times$,
with the caveats of \S\ref{sec:zeroshot}) --- it is
...
\begin{itemize}
  ...
  \item All in-domain headline results in \S\ref{sec:twinresults} concern
    a single flood event (Australia); the in-domain UK replication is
    in progress at the time of writing. The zero-shot transfer study
    (\S\ref{sec:zeroshot}) is evidence of cross-event robustness on two
    independent transfer pairs, evaluated on each target event's full
    frame sequence after an initial small-test-split pass gave inflated
    ratios (\S\ref{sec:zeroshot} states this pitfall explicitly); one
    protocol detail remains unverifiable (whether the published transfer
    numbers froze source-domain normalization as ours do), and the
    Pakistan$\to$Mozambique pair is single-seed pending 3-seed
    replication.
