...
has two independent transfer pairs (Australia$\to$UK, 3 seeds;
Pakistan$\to$Mozambique, 1 seed), both favoring the twin by two orders of
magnitude over the published FNO+ numbers. What remains before freeze is
the 3-seed replication of the Pakistan$\to$Mozambique pair, to bring it
to the same statistical footing as the UK one rather than changing this
...
\paragraph{Second transfer pair: Pakistan $\to$ Mozambique.} The
benchmark's \emph{other} published transfer pair (480\,m, low-fidelity;
published FNO+ transfer relRMSE 0.078633) gives an independent check.
Training the twin in-domain on Pakistan (no Pakistan checkpoint
previously existed; single seed, dense) and evaluating it zero-shot
(frozen weights and normalization) on Mozambique gives relRMSE
\textbf{0.000374} over the 7 eligible test windows --- $\sim$\textbf{210$\times$
better} than the published FNO+ transfer number, the same direction as
Table~\ref{tab:zeroshot} and, if anything, a larger margin. This is a
single-seed result (the Pakistan in-domain training this depends on has
not yet been replicated at 3 seeds, unlike the UK table above) on a
7-window test split --- smaller than we originally expected from this
split's frame count, so the same small-sample caveat applies, now to
both transfer pairs rather than being retired. Both published transfer
pairs now point the same direction, with the twin ahead in each; we
report this as convergent evidence across two independent regions rather
than a settled headline number, pending the 3-seed replication.
...
both published transfer pairs ($\sim$37$\times$ and $\sim$210$\times$,
...
\begin{itemize}
  ...
  \item All in-domain headline results in \S\ref{sec:twinresults} concern
    ...
    independent transfer pairs but rests on small test splits (3 and 7
    windows) and one unverifiable protocol detail (whether the published
    transfer numbers froze source-domain normalization as ours do); the
