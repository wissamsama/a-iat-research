...
\begin{enumerate}
  ...
  \item \textbf{A demonstrated failure mechanism} for absolute-field
    diffusion at fine cadence: a 450--490$\times$ target-scale mismatch
    measured on two independent events, confirmed by an 8-point
    dose--response test showing the predicted monotone decay (8/8) with
    a power-law exponent matching the model's prediction
    (\S\ref{sec:mechanism}).
...
range --- the pre-registered criterion, met without exception.
$\S$\ref{sec:mechanism}'s claim upgrades from hypothesis to
\textbf{demonstrated mechanism}. Beyond monotonicity, the ratio follows
the predicted power-law form closely: a log--log regression of ratio
against $\Delta t$ gives slope $-1.06$ ($R^2=0.98$), matching the $-1$
exponent the sampling-noise-floor model predicts (since $\sigma_\Delta$
grows near-linearly in $\Delta t$, Figure~\ref{fig:doseresponse}). We
report this fit as evidence the data are \emph{consistent with} the
predicted functional form, not as a precision measurement of the
exponent itself --- it comes from a single-seed screening protocol, and
the training-set size shrinks sharply at the largest $\Delta t$ values
(1579 training deltas at 300\,s vs.\ 65 at 7200\,s), so the extreme point
anchoring one end of the fit is also the least-supported by training
data. A secondary, unplanned finding: \ours{} itself (delta target) stops
beating persistence beyond $\Delta t\approx 65$\,min (interpolated
crossing $\approx$3910\,s) --- at that horizon genuine physical change
accumulates enough that ``predict no change'' is no longer a weak
baseline, independent of target encoding.
