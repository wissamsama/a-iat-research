...
better still, $\sim$9.4$\times$ below the published state of the art at
$\sim$1/320th the inference cost. Across two flood events, three sparsity
...
under full observation, at $\sim$1/320th the diffusion model's inference
cost, and never behind it with consistent sign in any regime we test
(\S\ref{sec:twinresults}, \S\ref{sec:ukresults}).
...
\begin{enumerate}
  \item \textbf{A new state of the art on FloodCastBench}: \ours{} beats
    ...
    further $\sim$3.7$\times$ at $\sim$1/320th the cost
    (\S\ref{sec:vsfnoplus}).
...
\blocked{Remaining before freeze: the Pakistan$\to$Mozambique transfer
pair at 3 seeds (currently 1; training in progress), the WP12 twin
control's final $\Delta t$ point (7 of 8 complete), the context ablation
at 3 seeds, and the ablation/cost study of \S\ref{sec:ablations}.}
...
anywhere. The twin reproduces the same power law --- slope $-0.91$,
$R^2=0.98$ over the 7 of 8 $\Delta t$ points complete at the time of
writing. \blocked{The final twin-control point ($\Delta t=7200$\,s) is
training; the fitted slope will be updated when it lands.} The
...
reaches relative RMSE 0.000202 vs.\ published FNO+ 0.002107
($\sim$10$\times$, ahead on all five metrics; single seed,
...
Pakistan$\to$Mozambique (85 windows) & FNO+ \citep{floodcastbench2025} & 0.078633 & --- \\
 & \twin{} (1 seed) & $\mathbf{0.003748}$ & $\sim$\textbf{21$\times$} \\
\bottomrule
\end{tabular}
\end{table}
...
$\sim$8$\times$ --- most of the $\sim$21$\times$ comes from FNO+
...
which barely does. Caveats: the Pakistan$\to$Mozambique row is single-seed
(3-seed replication in progress; the UK row's seed variance is
$\approx$6\% of its mean), and the publication does not specify whether
its transfer numbers froze source-domain normalization as ours do.
\blocked{\ours{}'s full-event transfer numbers and the Pakistan seed
...
\begin{figure}[H]
  ...
  structured masks (no random-mask run at that sparsity), which
  \S\ref{sec:masks} suggests understates rather than overstates its
  calibration.}
  \label{fig:calibcomp}
\end{figure}

\blocked{Remaining: a matched random-mask \ours{} evaluation at
\mninetyfive{}, and a sharpness-aware score (CRPS) to complement
coverage.}

\subsection{Ablations and cost}
\label{sec:ablations}
\blocked{Two studies planned: (i) a component ablation of \ours{} (delta
parametrization, target-step rainfall, spatial encoder, change-weighted
loss, rollout-aware training, diffusion steps); (ii) a measured cost
table (parameters, latency, memory, wall-clock) pricing \ours{}'s 40
steps $\times$ 8 scenarios against the twin's single pass.}
...
beats FNO+ in-domain ($\sim$9.4$\times$ and $\sim$10$\times$ on two
events) and on both published zero-shot transfer pairs
($\sim$8.6$\times$, $\sim$21$\times$). We answer deliberately narrowly.
...
\begin{itemize}
  ...
  \item In-domain results cover two events of one benchmark; zero-shot
    transfer covers its two published pairs, with the
    Pakistan$\to$Mozambique row single-seed (replication in progress)
    and one unverifiable protocol detail (whether published transfer
    numbers froze source normalization as ours do).
...
\begin{enumerate}
  ...
  \item \textbf{Convergence audit.} Sparse-regime checkpoints of
    \emph{both} models were under-trained under the original budget
    (retraining improved \ours{}'s internal selection metric by
    16--37\%). Every sparse number was re-derived on retrained
    checkpoints; the central verdict was unchanged
    ($\times$3.55/quasi-tie/$\times$1.57 before vs.\
    $\times$3.7/undecided/$\times$1.4 after), which we read as evidence
    the verdict is not an artifact of the correction.
...
$0.002874\pm0.000177$ ($\sim$8.6$\times$) and $0.003748$
($\sim$21$\times$) --- the numbers reported in
...
transfer claim. \ours{} on the same initial UK slice scored
$0.001850\pm0.000498$ ($\sim$13$\times$), consistently behind \twin{};
its full-event numbers were still running at the time of writing.
...
$-0.91$, $R^2=0.98$ (7 of 8 points at the time of writing) vs.\ the
diffusion arm's $-1.06$, $R^2=0.98$.
...
\paragraph{Pakistan in-domain (single seed, screening)} --- \twin{}
trained on Pakistan (480\,m, dense), full test protocol (20 windows),
vs.\ published low-fidelity FNO+ \citep{floodcastbench2025}: relative
RMSE $0.000202$ vs.\ $0.002107$; NSE $0.9999999$ vs.\ $0.999994$;
Pearson $r$ $0.9999999$ vs.\ $0.999997$; CSI@0.001 $0.9914$ vs.\
$0.976716$; CSI@0.01 $0.9986$ vs.\ $0.993993$ --- ahead on all five.

\paragraph{Mozambique zero-shot (full event, 85 windows, single seed)}
--- NSE $0.999984$, CSI@0.001 $0.9792$, alongside the relative RMSE of
Table~\ref{tab:zeroshot}; the twin beats oracle persistence by
$\sim$8.4$\times$ on normalized RMSE, so most of the $\sim$21$\times$
margin over FNO+ reflects FNO+ transferring worse than persistence
rather than headroom above it.
